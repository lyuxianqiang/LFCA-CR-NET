...
class Autocovnlayer(nn.Module):
    ...
    def forward(self,x,temperature_1,temperature_2):
        ...
        dence_weight_soft = torch.zeros(dence_weight.shape)
        dence_weight_soft[dence_weight > 0.5] = 1
        dence_weight_soft = dence_weight_soft[:,None,None,None,None,None].cuda()
        
        component_weight = self.component_weight.clamp(0.02,0.98)
        component_weight_gumbel = torch.zeros(component_weight.shape)
        component_weight_gumbel[component_weight > 0.5] = 1
        component_weight_gumbel = component_weight_gumbel[:,None,None,None,None,None].cuda()
