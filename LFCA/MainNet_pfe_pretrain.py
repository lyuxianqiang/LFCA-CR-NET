...
from RefNet_pfe_pretrain import RefNet
...
class MainNet(torch.nn.Module):
    def __init__(self,opt):
        ...
        # Shot layer    
        ##self.proj_init=torch.nn.Conv2d(in_channels=opt.channelNum,out_channels=opt.channelNum,kernel_size=self.kernelSize,bias=False)
    ...
    def forward(self, lf, epoch):
        ...
        _,m,_,_ = degLF.shape
        degLF = degLF.reshape(b,x,y,m,c).permute(0,3,4,1,2)  #[b,m,c,x,y]  m = udeg*vdeg
        # Initialize LF from measurements
        initLF = self.initialRefnet(degLF, epoch) #[buv,c,x,y]
        out=initLF
